...
bool CoveragePathPlanningHeuristic::terminateConditionReached(Node *node, bool DeclineCovMax, bool DeclineGainMax, double maxCov)
{
    ...
    // else if ((DeclineCovMax==1) && (maxCov >= (coverageTarget-5))) // This will stop the program once the total coverage starts to decrease
    // {
    //     std::cout<<"The path has reached the max coverage for this configuration:"<<maxCov<<" \n";
    //     return true;     
    // }
    // else if (DeclineGainMax==1)
    // {
    //     std::cout<<"The possible path coverage increase has fallen below 0.25 \n";
    //     return true; 
    // }
    // else if ((node->coverage-parCov) <= 0.25)
    // {
    //     std::cout<<"The possible path coverage increase has fallen below 0.25 \n";
    //     return true; 
    // }
...
//                f = node->totalEntroby + extraVolume + normDist +normAngle;
                ...
                // f = node->parent->f_value-c*std::exp(d*-0.2)+2*normAngle;//Added*(normAngle);
...
//                f = node->totalEntroby+ extraVolume +normAngle;
                f = node->parent->f_value+localE*std::exp(d*-0.2);//*(normAngle); ORIGINAL
                // f = node->parent->f_value-c*std::exp(d*-0.2)+2*normAngle;//Added*(normAngle);
                // f = node->parent->f_value+localE*std::exp(d*-0.2)+2*normAngle-c; //*(normAngle);
//                std::cout<<"heuristic value: "<<f<<"local Entropy: "<<localE<<" distance: "<<d<<std::endl;
//                std::cout<<"Total Entroby: "<<node->totalEntroby<<std::endl;
            }
        }
